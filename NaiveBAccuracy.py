...
def get_Probability(words):
    ...
    return (rating_Original, final_Probability)
...
if __name__ == "__main__":
    sc = SparkContext(appName="Naive-Bayes")# creating a sparkcontext object with application name "Naive-Bayes"
    ...
    len_Vocablry = len(wordsinTrainData)# Returns the length of the vocabulary present in train data
    ...
    one_Words2 = [y for x in one_Words1.collect() for y in x]# creates a list of words which belongs to rating group 2 from each tuple
    ...
    alpha = 1.0
	# Calculating probability of a word being in a class
    while(alpha<=5):
        ...
        classProbability = dict(ratingreviewlist.map(lambda x: (x[0], len([item for item in x[-1] if item]))).map(
            lambda (x, y): (x, (y * 1.0) / n)).collect())
        


        print "Calculate Accuracy of the Naive Bayes Model"

        WordsForTest = test_Data.map(lambda (x, y): (y[-1], y[-1] + " " + x)).map(lambda x: (x[0], (x[1:]))).map(
            lambda x: x[1]).flatMap(lambda xs: [(x, 1) for x in xs]).map(lambda x: x[0].split(' ')).map(
            lambda x: get_Probability(x))
        predictions = WordsForTest.collect()
        accuracy = get_RvwAccuracy(predictions)


        print "============================================================================================="
        print('NaiveBayes accuracy for alpha '+ repr(alpha)+ ' is ' + repr(accuracy) + '%')
        alpha = alpha + 1
        print "============================================================================================="
